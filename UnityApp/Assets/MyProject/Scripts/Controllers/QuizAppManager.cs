...
public class QuizAppManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        selectedCategories = GameManager.Instance.categoriesList;

        //selectedWords = SelectRandomWords();
    ...
    List<Question> GenerateQuestions(List<Word2> selectedWords)
    {
        ...
        for (int i = 0; i < words.Count && i < selectedWords.Count; i++)
        {
    ...
    public void OnAnswerSelected(int choiceIndex)
    {
        ...
        */
        ...
        if (currentQuestion.IsCorrect) { 
            correctAnswers++;
            FirebaseManager.Instance.SaveExerciseProgress(FirebaseManager.Instance.user.UserId, level, correctAnswers);
    ...
    public void OnNextButtonClicked()
    {
        if (currentQuestionIndex == questions.Count-1)
        {
            //Showing the result and make prediction too
    ...
    void ConvertQuestionsToDataEntries(List<Question> questions)
    {
        ...
        StartCoroutine(GameManager.Instance.apiManager.PredictData(dataEntries, OnPredictionsReceived));
        //WeaknessCalculator.CalculateWeakness(dataEntries);
    }
    void OnPredictionsReceived(List<TrainingData> predictions, List<DataEntry> dataEntries)
    {
        if (predictions != null)
        {
            ...
            {
                dataEntries[i].Weakness = predictions[i].label;
            }
            List<CategoryWeakness> weaknesses = GameManager.Instance.weaknessCalculator.GetCategoryWeakness(dataEntries);
            Debug.Log("weaknesses count : "+weaknesses.Count);
            FirebaseManager.Instance.SaveCategoryWeakness(FirebaseManager.Instance.user.UserId, weaknesses);
